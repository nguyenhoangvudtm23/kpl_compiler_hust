...
void compileProgram(void) {
    ...
    eat(KW_PROGRAM);
    eat(TK_IDENT);
    Object *object = createProgramObject(currentToken->string);
    eat(SB_SEMICOLON);
    // before compile block need to enter
    enterBlock(object->progAttrs->scope);
    compileBlock();
    // after finishing compile block, need exit
    exitBlock();
    eat(SB_PERIOD);
}
...
void compileBlock(void) {
    ...
    if (lookAhead->tokenType == KW_CONST) {
        ...
        do {
            eat(TK_IDENT);
            object = createConstantObject(currentToken->string);
            eat(SB_EQ);
            object->constAttrs->value = compileConstant();
            eat(SB_SEMICOLON);
...
void compileBlock2(void) {
    ...
    if (lookAhead->tokenType == KW_TYPE) {
        ...
        do {
            eat(TK_IDENT);
            object = createTypeObject(currentToken->string);
            eat(SB_EQ);
            object->typeAttrs->actualType = compileType();
            eat(SB_SEMICOLON);
            declareObject(object);
        } while (lookAhead->tokenType == TK_IDENT);
...
void compileBlock3(void) {
    ...
    if (lookAhead->tokenType == KW_VAR) {
        ...
        do {
            eat(TK_IDENT);
            object = createVariableObject(currentToken->string);

            eat(SB_COLON);
            object->varAttrs->type = compileType();
...
void compileFuncDecl(void) {
    ...
    eat(KW_FUNCTION);
    eat(TK_IDENT);
    lookupObject(currentToken->string);
    // check if exits func in current scope
    object = findDuplicateIdent(currentToken->string);
    if (object != NULL){
        error(ERR_INVALID_FUNCTION, lookAhead->lineNo, lookAhead->colNo);
    }
...
void compileProcDecl(void) {
    ...
    object = findDuplicateIdent(currentToken->string);
    if(object != NULL){
        error(ERR_INVALID_PROCEDURE, lookAhead->lineNo, lookAhead->colNo);
    }
...
ConstantValue* compileUnsignedConstant(void) {
    ...
    switch (lookAhead->tokenType) {
        ...
        case TK_IDENT:
            eat(TK_IDENT);
            Object* object = lookupObject(currentToken->string);
...
Type* compileType(void) {
    ...
    switch (lookAhead->tokenType) {
        ...
        case KW_ARRAY:
            ...
            eat(SB_LSEL);
            eat(TK_NUMBER);
            int arr_size =currentToken->value;
            eat(SB_RSEL);
            eat(KW_OF);
            Type *elementType = compileType();
            type = makeArrayType(arr_size, elementType);
            break;
        case TK_IDENT:
            eat(TK_IDENT);
            Object *object = lookupObject(currentToken->string);
